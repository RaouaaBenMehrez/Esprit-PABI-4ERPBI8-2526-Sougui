...
app = FastAPI(title="Boutique Artisanale — Agent IA", version="4.0.0")
...
def health():
    ...
    if api_key and api_key != "ta_clé_gemini_ici":
        try:
            ...
            if genai.GenerativeModel("gemini-2.5-flash").generate_content("OK").text:
...
def chat(req: ChatRequest):
    """
    Séparation claire des 3 cas :

    "Bonjour"                    → intention: simple     (pas de RAG)
    "cadeau céramique 30 DT"     → intention: recommandation (RAG si assez d'infos)
    "je cherche quelque chose"   → intention: recommandation (1 question d'abord)
    "ma commande est abîmée"     → intention: sav        (pas de RAG)

    Postman examples:
    {"session_id":"s1","message":"Bonjour"}
    {"session_id":"s1","message":"je cherche un cadeau pour ma mère"}
    {"session_id":"s1","message":"quelque chose en céramique, budget 30 DT","budget_max":30}
    {"session_id":"s1","message":"ma commande est abîmée"}
    """
...
def reset_session(session_id: str):
    ...
    # Optionnel: on pourrait aussi supprimer de la DB
    return {"success": True}
